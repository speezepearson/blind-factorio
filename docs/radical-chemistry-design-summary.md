> **Snapshot note:** copied into the repo on 2026-08-17 so the design travels with the code.
> The editable originals live in `~/` on the dev VM (`radical-chemistry-design-summary.md`,
> plus the reference prototypes `radical-kinetics-sandbox.jsx` and `radical-veins-game.jsx`).
> If this drifts from the original, the original wins.
